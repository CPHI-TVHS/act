...
MALLET_DIR = roles_config.MALLET_DIR
...
def run_prepare_train(corpus_root, corpus, start_range, end_range, cat_type, subset):

    for int_year in range(start_range, end_range):
        year = str(int_year)
        print "[run_my_func]Processing dir: %s" % year
        prepare_train(corpus_root, corpus, year, cat_type, subset)
        print "[run_my_func]Completed: %s.tc" % (year)


def run_prepare_classify(corpus_root, corpus, start_range, end_range, cat_type, subset):

    for int_year in range(start_range, end_range):
        year = str(int_year)
        print "[run_prepare_classify]Processing dir: %s" % year
        prepare_classify(corpus_root, corpus, year, cat_type, subset)
