...
Good elasticsearch info: https://www.found.no/foundation/elasticsearch-from-the-bottom-up/
...
cd /home/j/anick/patent-classifier/ontology/roles/data/patents/ln-us-A27-molecular-biology/data/term_features/2003
[anick@sarpedon 2003]$ ls -1 | wc -l
38343

# bigrams and trigrams are stored here in subdirectories named <domain>_<year>:
cd /home/j/anick/patent-classifier/ontology/roles/data/nc

# eval data summarized over multiple years goes here:
...
# restarted (on sarpedon, cd /indexes/elasticsearch/bin)
[anick@sarpedon bin]$ /usr/local/bin/elasticsearch

NOTE! This restarts in foreground, so don't do it (cntl-C to cancel)
...
Defintion of termination codon includes translation, so BC is a shorter form of ABC
...
5/5/15
ISSUE: After looking at npmi results (from running run_pmi in pmi.py)
We compute using bigram frequency.  Does it make a difference if the bigrams are first restricted to strictly Noun compounds
vs. nouns and adj-noun compounds?  Since other people are using only NN, we should at least do the same.  However, if the trigram is an NNN,
then the bigram pairs can be assumed to be NN as well.  But the (A,X) and (X, B) frequencies might include both Adj and Noun compounds, unless 
ADj are excluded from the bigram lists used to compute freq. These are used for computing PMI.

We should try computing both ways. For now, let's include all bigram types.

Directories used:

/home/j/anick/patent-classifier/ontology/roles/data/nc/eval/bio_<year>   data for a single patent domain and year
/home/j/anick/patent-classifier/ontology/roles/data/nc/eval  data that combines multiple years
/home/j/anick/patent-classifier/ontology/roles/data/patents/ln-us-A27-molecular-biology/data/tv_tas for term/feature data for different years

For fuse roles:
use .tf file to get term and feature counts for a year
to get growth terms, need doc freq from year to year  .terms 

Given .terms for 3 years, e.g. 1997, 2000, 2003
compute prob(growth_term | feature) = prob(feature | growth_term) * prob(growth_term) / prob(feature)

Need to compute PMI of feature term occurrences

Marc has also loaded pubmed which we can use for medical docs analysis.  Need to test out the different fields it has. 

5/9/2015
Put annotation file in final form with | separator (in /home/j/anick/patent-classifier/ontology/roles/data/nc/eval)
cat growth.bio_1997_2003.stats.cat.1_10.N.3300.annot | sed -e 's/$/ \| /' > bio_NNN.to_annotate

I notice that there are not 3300 lines in the NNN annotation file.  This is because there are not enough NNN phrases that satisfy the
freq constraints.  Let's try again with 2003 freq >= 8.

# use awk to apply the numeric constraints to columns 4 and 14.  We take terms with count of 1 in 1997 and >=8 in 2003.

cat growth.bio_1997_2003.stats.cat | awk -F $'\t' 'BEGIN {OFS = FS} $4 == 1 && $14>=8'  > growth.bio_1997_2003.stats.cat.1_8
[anick@sarpedon eval]$ cat growth.bio_1997_2003.stats.cat | awk -F $'\t' 'BEGIN {OFS = FS} $4 == 1 && $14>=8'  > growth.bio_1997_2003.stats.cat.1_8
# how many total? (adj and noun compounds)
[anick@sarpedon eval]$ cat growth.bio_1997_2003.stats.cat.1_8 | wc -l
8998
# how many are NNN compounds?
[anick@sarpedon eval]$ cat growth.bio_1997_2003.stats.cat.1_8 | grep NNN | wc -l
3563

# divide up the triples into J and N files
cat growth.bio_1997_2003.stats.cat.1_8 | grep -v J > growth.bio_1997_2003.stats.cat.1_8.N
cat growth.bio_1997_2003.stats.cat.1_8 | grep J > growth.bio_1997_2003.stats.cat.1_8.J
[anick@sarpedon eval]$ wc -l growth.bio_1997_2003.stats.cat.1_8.N
3563 growth.bio_1997_2003.stats.cat.1_8.N
[anick@sarpedon eval]$ wc -l growth.bio_1997_2003.stats.cat.1_8.J
5435 growth.bio_1997_2003.stats.cat.1_8.J

Put annotation file in final form with | separator (in /home/j/anick/patent-classifier/ontology/roles/data/nc/eval)
Note the tab <t> needs to be entered manually.
cat growth.bio_1997_2003.stats.cat.1_8.N.3300.annot | sed -e 's/$/<t>\| /' > bio_NNN.to_annotate

Move it to my mac (in panick directory)
cp cp bio_NNN.to_annotate ~
new-host-2:~ panick$ scp anick@sarpedon.cs.brandeis.edu:bio_NNN.to_annotate .

To make a windows file: (from https://kb.iu.edu/d/acux)
...
HEALTH
##################
6/7/15  creating indices for health domain 1997 and 2002

python2.7
import es_np_index

>>> es_np_index.np_populate("i_health_1997", "health", "ln-us-14-health", 1997, 1997, 5000, True, True, 0) 
Sun Jun  7 18:55:36 2015        0       [es_np_index.py]Completed make_bulk_lists for years: 1997 1997. Number of lines: 52856156

[gen_bulk_lists]52856156 lines from 20124 files written to index i_health_1997
[es_np_index.py] Bulk loaded sublist 8740
[es_np_index.py] bulk loading completed
[es_np_index.py] index refreshed
[se_np_index.py]np_populate completed at Sun Jun  7 18:55:37 2015
 (elapsed time in hr:min:sec: 3:39:42.802355)

>>> es_np_index.np_populate("i_health_2002", "health", "ln-us-14-health", 2002, 2002, 5000, True, True, 0)

This gave me 1150 bulk loads (at 5000 lines each).  Then I got:
...
Traceback (most recent call last):
  File "<stdin>", line 1, in <module>
  File "es_np_index.py", line 581, in np_populate
    res = es.bulk(l_bulk_elements)
  File "/usr/lib/python2.7/site-packages/elasticsearch/client/utils.py", line 70, in _wrapped
    return func(*args, params=params, **kwargs)
  File "/usr/lib/python2.7/site-packages/elasticsearch/client/__init__.py", line 568, in bulk
...
So I restarted with a small bulk size (3000)
>>> es_np_index.np_populate("i_health_2002", "health", "ln-us-14-health", 2002, 2002, 3000, True, True, 0)
This still failed at bulk # 1933, around the same place as before (~5799k lines)
I tried again, after closing python, which might release some open files.
Another possibility is to raise ulimit:  http://elasticsearch-users.115913.n3.nabble.com/Error-Too-many-open-files-td2779067.html

I tried again after closing and reopening python.  This time the error was in a slightly different place:

[es_np_index.py] Bulk loaded sublist 1890
WARNING:elasticsearch:POST http://localhost:9200/_bulk [status:N/A request:10.011s]
Traceback (most recent call last):
  File "/usr/lib/python2.7/site-packages/elasticsearch/connection/http_urllib3.py", line 46, in perform_request
    response = self.pool.urlopen(method, url, body, **kw)
  File "/usr/lib/python2.7/site-packages/urllib3/connectionpool.py", line 541, in urlopen
...
#######################
Monday: Ask Chris to do:
raise the no_files of the user running elasticsearch, this is done
typically adding to /etc/security/limits.conf the lines:

elasticsearch soft nofile 64000
elasticsearch hard nofile 64000

test that it gets applied running:

sudo -u elasticsearch -s "ulimit -Hn"

If it doesn't get applied run:

egrep -r pam_limits /etc/pam.d/

and check that all pam_limits.so are not commented out.

Also stop and restart elasticsearch on both machines (sarpedon and ///)
The above changes fixed the ReadTimeoutError.  i_health_2002 completed.

! Need to make all data on sarpedon available on scaroth.cs-i.brandeis.edu

###########
i_health_2002 bulk load completed:

[es_np_index.py] Bulk loaded sublist 17035
Mon Jun  8 15:47:24 2015        0       [es_np_index.py]Completed make_bulk_lists for years: 2002 2002. Number of lines: 60263843

[gen_bulk_lists]60263843 lines from 19817 files written to index i_health_2002
[es_np_index.py] Bulk loaded sublist 17036
[es_np_index.py] bulk loading completed
[es_np_index.py] index refreshed
[se_np_index.py]np_populate completed at Mon Jun  8 15:47:25 2015
 (elapsed time in hr:min:sec: 3:23:47.808274)
...
# bigrams
es_np_query.dump_ngrams("i_health2_1997", 2, "/home/j/anick/patent-classifier/ontology/roles/data/nc/health_1997/bigrams.inst")  
wc -l bigrams.inst
36604883 bigrams.inst

# trigrams
es_np_query.dump_ngrams("i_health2_1997", 3, "/home/j/anick/patent-classifier/ontology/roles/data/nc/health_1997/trigrams.inst")  

# Do the filtering of illegal phrases (all folded into a single function now)
>>> es_np_nc.filter_phr_doc_file("/home/j/anick/patent-classifier/ontology/roles/data/nc/health_1997/bigrams.inst")

>>> es_np_nc.filter_phr_doc_file("/home/j/anick/patent-classifier/ontology/roles/data/nc/health_1997/trigrams.inst")

wc -l trigrams.inst.filt
wc -l bigrams.inst.filt

# sort uniq to get document frequencies rather than collection frequencies
cd /home/j/anick/patent-classifier/ontology/roles/data/nc/health_1997
# At this point, we throw away the surface form of the term amd keep the
# canonical form only.
cat trigrams.inst.filt | cut -f1,3 | sort | uniq > trigrams.inst.filt.su
cat bigrams.inst.filt | cut -f1,3 | sort | uniq > bigrams.inst.filt.su
...
# health_2002
# bigrams
es_np_query.dump_ngrams("i_health_2002", 2, "/home/j/anick/patent-classifier/ontology/roles/data/nc/health_2002/bigrams.inst")  
wc -l bigrams.inst
36604883 bigrams.inst

# trigrams
es_np_query.dump_ngrams("i_health_2002", 3, "/home/j/anick/patent-classifier/ontology/roles/data/nc/health_2002/trigrams.inst")  


# Do the filtering of illegal phrases (all folded into a single function now)
>>> es_np_nc.filter_phr_doc_file("/home/j/anick/patent-classifier/ontology/roles/data/nc/health_2002/bigrams.inst")

>>> es_np_nc.filter_phr_doc_file("/home/j/anick/patent-classifier/ontology/roles/data/nc/health_2002/trigrams.inst")
...
# sort uniq to get document frequencies rather than collection frequencies
cd /home/j/anick/patent-classifier/ontology/roles/data/nc/health_2002
# At this point, we throw away the surface form of the term amd keep the
# canonical form only.
cat trigrams.inst.filt | cut -f1,3 | sort | uniq > trigrams.inst.filt.su
cat bigrams.inst.filt | cut -f1,3 | sort | uniq > bigrams.inst.filt.su
...
# BUG: Only some indixes are available to access by elasticsearch.  e.g. i_health_2002 is visible but i_health_1997 is not, after
Chris changed a ulimit and heap size parameter and restarted

I will try shutting down all nodes and restarting.
...
Note that the heap size parameters Chris supposedly added are not used!  

[anick@sarpedon index]$ ps aux | grep elastic
anick    32208  397  0.6 17091068 447108 pts/9 Sl   18:39   4:02 /usr/bin/java -Xms256m -Xmx1g -Xss256k -Djava.awt.headless=true -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75 -XX:+UseCMSInitiatingOccupancyOnly -XX:+HeapDumpOnOutOfMemoryError -XX:+DisableExplicitGC -Delasticsearch -Des.path.home=/indexes/elasticsearch -cp :/indexes/elasticsearch/lib/elasticsearch-1.4.1.jar:/indexes/elasticsearch/lib/*:/indexes/elasticsearch/lib/sigar/* org.elasticsearch.bootstrap.Elasticsearch

Let's restart with heap size parameters explicitly added to the command.
/usr/local/bin/elasticsearch -d -Xmx4g -Xms4g

///This no longer works!
anick    32610  418  2.2 20287892 1686168 pts/9 Sl  18:45   1:57 /usr/bin/java -Xms256m -Xmx1g -Xss256k -Djava.awt.headless=true -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75 -XX:+UseCMSInitiatingOccupancyOnly -XX:+HeapDumpOnOutOfMemoryError -XX:+DisableExplicitGC -Delasticsearch -Des.path.home=/indexes/elasticsearch -cp :/indexes/elasticsearch/lib/elasticsearch-1.4.1.jar:/indexes/elasticsearch/lib/*:/indexes/elasticsearch/lib/sigar/* -Xmx4g -Xms4g org.elasticsearch.bootstrap.Elasticsearch

# check on indexes available
curl 'localhost:9200/_cat/indices?v'

Apparently, es can only access indices in node 0:
/indexes/elasticsearch/data/elasticsearch/nodes/0/indices

While other indices in node 2 are not seen.

/// here are a list of configuration changes that should be made:
https://www.elastic.co/guide/en/elasticsearch/guide/master/_important_configuration_changes.html

I may need to recreate all my indexes on sarpedon
First let's get the heap size setup
-bash-4.1$ echo $ES_HEAP_SIZE

-bash-4.1$ export ES_HEAP_SIZE=4g
-bash-4.1$ echo $ES_HEAP_SIZE
4g

[anick@sarpedon indices]$ export ES_HEAP_SIZE=4g
[anick@sarpedon indices]$ echo $ES_HEAP_SIZE
4g
[anick@sarpedon indices]$ curl -XPOST 'http://localhost:9200/_shutdown'
{"cluster_name":"elasticsearch","nodes":{"xUgcn4xUTc-yPrqNxrky_Q":{"name":"Calvin Rankin"}}}[anick@sarpedon indices]$ 
[anick@sarpedon indices]$ 
[anick@sarpedon indices]$ 
[anick@sarpedon indices]$ /usr/local/bin/elasticsearch -d -Xmx4g -Xms4g
[anick@sarpedon indices]$ ps aux | grep elastic
anick     1970  243  1.2 19693408 940216 pts/9 Sl   19:34   0:17 /usr/bin/java -Xms4g -Xmx4g -Xss256k -Djava.awt.headless=true -XX:+UseParNewGC -XX:+UseConcMarkSweepGC -XX:CMSInitiatingOccupancyFraction=75 -XX:+UseCMSInitiatingOccupancyOnly -XX:+HeapDumpOnOutOfMemoryError -XX:+DisableExplicitGC -Delasticsearch -Des.path.home=/indexes/elasticsearch -cp :/indexes/elasticsearch/lib/elasticsearch-1.4.1.jar:/indexes/elasticsearch/lib/*:/indexes/elasticsearch/lib/sigar/* -Xmx4g -Xms4g org.elasticsearch.bootstrap.Elasticsearch
anick     2190  0.0  0.0 103248   896 pts/9    S+   19:34   0:00 grep elastic

# Now try re-indexing the health and bio indexes under new names
>>> es_np_index.np_populate("i_health2_1997", "health", "ln-us-14-health", 1997, 1997, 5000, True, True, 0)
...
[es_np_index.py] Bulk loaded sublist 8739
Mon Jun  8 22:56:02 2015        0       [es_np_index.py]Completed make_bulk_lists for years: 1997 1997. Number of lines: 52856156

[gen_bulk_lists]52856156 lines from 20124 files written to index i_health2_1997
[es_np_index.py] Bulk loaded sublist 8740
[es_np_index.py] bulk loading completed
[es_np_index.py] index refreshed
[se_np_index.py]np_populate completed at Mon Jun  8 22:56:03 2015
 (elapsed time in hr:min:sec: 2:46:11.687927)

>>> es_np_index.np_populate("i_health2_2002", "health", "ln-us-14-health", 2002, 2002, 5000, True, True, 0)

status of indices:
[anick@sarpedon bio_2003]$ curl 'localhost:9200/_cat/indices?v'
health status index          pri rep docs.count docs.deleted store.size pri.store.size 
red    open   i_health_2002    5   1                                                   
yellow open   i_bio            5   1        101            0     34.6kb         34.6kb 
yellow open   i_health2_1997   5   1   52732254            0      7.4gb          7.4gb 
yellow open   i_health2_2002   5   1     375842            0       67mb           67mb 
yellow open   test1            5   1          3            0     12.5kb         12.5kb 
yellow open   i_testg          5   1        101            0     34.6kb         34.6kb 
yellow open   computers        5   1     941572          632    166.3mb        166.3mb 
yellow open   i_nps_bio        5   1   39392738            0      4.6gb          4.6gb 
yellow open   i_np_bio         5   1   39392738            0      4.4gb          4.4gb 
yellow open   i_nps            5   1      11001            0      1.8mb          1.8mb 
yellow open   test2            5   1          3            0     15.9kb         15.9kb 
yellow open   test_stemming    5   1          4            0     13.2kb         13.2kb 
yellow open   test3            5   1          3            0        7kb            7kb 
yellow open   i_np             5   1    5750370          457    735.6mb        735.6mb 

[es_np_index.py] Bulk loaded sublist 10276
Tue Jun  9 03:04:34 2015        0       [es_np_index.py]Completed make_bulk_lists for years: 2002 2002. Number of lines: 60263843

[gen_bulk_lists]60263843 lines from 19817 files written to index i_health2_2002
[es_np_index.py] Bulk loaded sublist 10277
[es_np_index.py] bulk loading completed
[es_np_index.py] index refreshed
[se_np_index.py]np_populate completed at Tue Jun  9 03:04:34 2015
 (elapsed time in hr:min:sec: 3:22:26.893128)

Now try to create the full bio 1997 index:
>>> es_np_index.np_populate("i_bio2_1997", "biology", "ln-us-A27-molecular-biology", 1997, 1997, 5000, True, True, 0)

[es_np_index.py] Bulk loaded sublist 6969
Tue Jun  9 09:22:37 2015        0       [es_np_index.py]Completed make_bulk_lists for years: 1997 1997. Number of lines: 42859083

[gen_bulk_lists]42859083 lines from 11894 files written to index i_bio2_1997
[es_np_index.py] Bulk loaded sublist 6970
[es_np_index.py] bulk loading completed
[es_np_index.py] index refreshed
[se_np_index.py]np_populate completed at Tue Jun  9 09:22:38 2015
 (elapsed time in hr:min:sec: 2:34:54.201443)

>>> es_np_index.np_populate("i_bio2_2002", "biology", "ln-us-A27-molecular-biology", 2002, 2002, 5000, True, True, 0)

#####
6/11/15 populating CS databases

cd /home/j/anick/patent-classifier/ontology/roles/data/patents/ln-us-A21-computers/data
mkdir tv
(note that polarity data is in tv_tas_cond and tv_tas_cond/2 (for the second seed round output)
python2.7
>>> import es_np_index
>>> es_np_index.np_populate("i_computers2_2002", "computers", "ln-us-A21-computers", 2002, 2002, 5000, True, True, 0)

Loaded:
[es_np_index.py] Bulk loaded sublist 17578
Fri Jun 12 14:41:15 2015        0       [es_np_index.py]Completed make_bulk_lists for years: 2002 2002. Number of lines: 103989515

[gen_bulk_lists]103989515 lines from 45431 files written to index i_computers2_2002
[es_np_index.py] Bulk loaded sublist 17579
[es_np_index.py] bulk loading completed
[es_np_index.py] index refreshed
[se_np_index.py]np_populate completed at Fri Jun 12 14:41:15 2015
 (elapsed time in hr:min:sec: 6:02:00.768862)


June 17, 2015
sentence cooccurrence: es_np_query.q_corpus_doc_sent_cooccur
assoc metrics: pmi.py:class NCAssoc()
bigram assoc metrics computed by pmi.py are in /home/j/anick/patent-classifier/ontology/roles/data/nc/bio_1997 and bio_2003
es_np_nc.py has nc_classify, which compared stats for 1997 and 2003 to create our annotation sample.
We need to update this function to use the new assoc stats.

directories:
Until we merge data across years, data is kept in /home/j/anick/patent-classifier/ontology/roles/data/nc/bio_2003/ and bio_1997
Merged data goes to /home/j/anick/patent-classifier/ontology/roles/data/nc/eval/

es_np_nc.run_nc_classify does the merging.

BUG?  count in /home/j/anick/patent-classifier/ontology/roles/data/nc/bio_1997/bigrams.inst.filt.su.f1.uc1.nr
doesn't match count in bigrams.assoc

[anick@sarpedon bio_1997]$ cat /home/j/anick/patent-classifier/ontology/roles/data/nc/bio_1997/bigrams.inst.filt.su.f1.uc1.nr | grep "full-length cdna"
518     full-length cdna
[anick@sarpedon bio_1997]$ cat bigrams.assoc.f6 | grep "full-length cdna"
full-length cdna        1831    9267    18686   7.6421  0.5732  0.0007  0.0980  362632.5052

Fixed bug.  I had reused the same NCAssoc class instance to compute bigram data for 1997 and 2003 so they summed frequencies.
Corrected output:
[anick@sarpedon bio_1997]$ cat bigrams.assoc | grep "full-length cdna"
full-length cdna62      4       1791    4       10.9976 0.5553  0.0000  1.0000  8174.3361
full-length cdna        518     1791    4754    7.7995  0.6099  0.0011  0.1090  114570.7710

Added pos and corpus freq info to trigram and bigram files (in pmi.py)
>>> pmi.dump_canon_gram_freq("/home/j/anick/patent-classifier/ontology/roles/data/nc/bio_1997/trigrams.inst.filt")
>>> pmi.dump_canon_gram_freq("/home/j/anick/patent-classifier/ontology/roles/data/nc/bio_1997/bigrams.inst.filt")
This creates .freq files containing term corpus_freq doc_freq pos_signature
We continue to use doc_freq for our experiments.

Interesting data set in terms of differences between adj and dep models:

[anick@sarpedon bio_1997]$ cat trigrams.test.cell.pred | more
human cell line 357     234     JNN     R       R       R       R       L       L       L       L       702     2346    0       0.0121940627768 0.9
44824808699     0.0     0.130560403653  0.350912931263  0.0     635.539606485   6999.9690216    0.0
stem cell line  6       6       NNN     R       R       R       R       L       L       L       L       472     2346    0       0.00819885702375  0.944824808699   0.0     0.13203934266   0.350912931263  0.0     509.529720831   6999.9690216    0.0
ceramide treated cell   2       2       NJN     R       R       R       R       U       U       U       U       0       217     0       0.0     0.0
0376938977575   0.0     0.0     0.116124227809  0.0     0.0     235.168330413   0.0
cell structure test     1       1       NNN     L       L       L       L       L       R       R       R       44      0       3       0.239130434
783     0.0     0.75    0.000799171433832       0.0     0.108480243891  0.00230971833289        0.0     5.79687206986
cerebellar purkinje cell        13      13      NNN     R       R       R       R       R       R       R       R       0       18      1       0.0
        0.000312668276329       1.73704597961e-05       0.0     0.0771444774116 0.0627003475643 0.0     16.603450674    1.08177178068

Added pos and corpus freq info to trigram and bigram files (in pmi.py) for bio_2003
>>> pmi.dump_canon_gram_freq("/home/j/anick/patent-classifier/ontology/roles/data/nc/bio_2003/bigrams.inst.filt")
This creates .freq files containing term corpus_freq doc_freq pos_signature
Also do the same for trigrams:
pmi.dump_canon_gram_freq("/home/j/anick/patent-classifier/ontology/roles/data/nc/bio_2003/trigrams.inst.filt")


[anick@sarpedon bio_2003]$ pwd
/home/j/anick/patent-classifier/ontology/roles/data/nc/bio_2003

Cases where prediction differs between freq and x2:
cat trigrams.pred.sum | grep fLL | grep xRR | more
In many cases, freq does better.  Why?  Does chi2 do worse on lower freq terms?

Cases where adj and dep disagree:
[anick@sarpedon bio_2003]$ cat trigrams.pred.sum | grep -v J | grep fLR | wc -l
17866
[anick@sarpedon bio_2003]$ cat trigrams.pred.sum | grep -v J | grep fRL | wc -l
78849
[anick@sarpedon bio_2003]$ cat trigrams.pred.sum | grep J | grep fLR | wc -l
22325
[anick@sarpedon bio_2003]$ cat trigrams.pred.sum | grep J | grep fRL | wc -l
67774

totals:
[anick@sarpedon bio_2003]$ cat trigrams.pred.sum | grep J | wc -l
918754
[anick@sarpedon bio_2003]$ cat trigrams.pred.sum | grep -v J | wc -l
572460

To look at changes to a model prediction over time:
[anick@sarpedon eval]$ cat pred_bio_1997_2003.diff | grep NNN | grep PD | cut -f1,3,8,15,16,17,18 | grep fdRL | wc -l
1217
[anick@sarpedon eval]$ cat pred_bio_1997_2003.diff | grep NNN | grep PD | cut -f1,3,8,15,16,17,18 | grep fdLR | wc -l
1301
[anick@sarpedon eval]$ cat pred_bio_1997_2003.diff | grep NNN | grep PD | cut -f1,3,8,15,16,17,18 | grep xdRL | wc -l
2099
[anick@sarpedon eval]$ cat pred_bio_1997_2003.diff | grep NNN | grep PD | cut -f1,3,8,15,16,17,18 | grep xdLR | wc -l
2047

# terms where f and x disagree.
[anick@sarpedon eval]$ cat pred_bio_1997_2003.diff | grep NNN | grep PD | cut -f1,3,8,15,16,17,18 | grep xdRL | grep fdRR | wc -l
910

Try looking for local bigrams for rare trigrams.
Given ABC in doc D, does AB, BC, AC exist in doc D?  Does Ax, Bx, xB, xC exist?

We can get docs for trigrams from trigrams.inst.filt (in /home/j/anick/patent-classifier/ontology/roles/data/nc/bio_1997)

Discrepancy analysis
What is the prob of a discrepancy between adj and dep given the head term, middle and first?
Do this for N** and J** over trigrams.pred.sum data
Also for trigram freq ranges (1,10,100, 1000, more)

for freq and chi2 pairs LL, LR, RR, RL

# 
annotation file location
 cat bio_NNN.to_annotate.pa | grep '^[a-z]' | more

6/24/15 Another look at the Nakov evaluation data
/home/j/anick/patent-classifier/ontology/roles/data/nc/eval/Nakov_3nc.gold.labels.sorted

We have a collection of predictions for triples for bio_2003 in
/home/j/anick/patent-classifier/ontology/roles/data/nc/bio_2003/trigrams.pred

We can canonicalize the Nakov terms and look for them in the trigrams.pred file.

Here is the distribution of the Nakov gold set:
cat /home/j/anick/patent-classifier/ontology/roles/data/nc/eval/Nakov_3nc.gold.labels.sorted | cut -f1 | sort | uniq -c 
    361 L  84% out of 429
     68 R

Of the patent annotation set in /home/j/anick/patent-classifier/ontology/roles/data/nc/eval
I had annotated bio_NNN.to_annotate.pa (173 out of 3301 NNN trigrams)
I convert these into LR labeled form (label tab trigram):
1032  cat bio_NNN.to_annotate.pa.br | grep " \[" | sed -e 's/\[//' | sed -e 's/\]//' | cut -f1 | sed -e 's/^/R /' > bio_NNN.to_annotate.pa.br.R
 1034  cat bio_NNN.to_annotate.pa.br | grep "^\[" | sed -e 's/\[//' | sed -e 's/\]//' | cut -f1 | sed -e 's/^/L /' > bio_NNN.to_annotate.pa.br.L
This gives us:
[anick@sarpedon eval]$ wc -l bio_NNN.to_annotate.pa.br.L
129 bio_NNN.to_annotate.pa.br.L
[anick@sarpedon eval]$ wc -l bio_NNN.to_annotate.pa.br.R
44 bio_NNN.to_annotate.pa.br.R


On dja annotations, I think these should be changed to n:

leukocyte adhesion      pn      c 0.963596731386463
tgf-β activity  pn

### 8/4/15 looking at nakov.eval results in /home/j/anick/patent-classifier/ontology/roles/data/nc/bio_2003/nakov.eval
Most code is in pmi.py (e.g. produced pred.sum file)
Also some eval code in nc_eval.py (run_pi_nakov)
